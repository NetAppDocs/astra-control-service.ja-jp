...
Astra Trident:: ストレージバックエンドでの運用には、Astra Tridentと外部のSnapshotコントローラが必要です。インストールするには、次の手順を実行します。
...
まだ行っていない場合は、 https://docs.netapp.com/us-en/trident/trident-use/vol-snapshots.html#deploying-a-volume-snapshot-controller["スナップショットCRDとスナップショットコントローラをインストールします"^]。
