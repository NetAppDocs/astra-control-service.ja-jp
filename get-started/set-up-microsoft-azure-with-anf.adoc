...
[role="quick-margin-para"]
ネットアップリソースプロバイダを登録  for Azure NetApp Files,このステップの詳細をご覧ください。
...
[role="quick-margin-para"]
Azure ポータルで Azure NetApp Files にアクセスし、ネットアップアカウントを作成します。  a NetApp account,このステップの詳細をご覧ください。
...
[role="quick-margin-para"]
永続ボリューム用に 1 つ以上の容量プールを設定します。  up a capacity pool,このステップの詳細をご覧ください。
...
[role="quick-margin-para"]
サブネットを Azure NetApp Files に委譲し、 Astra Control サービスがそのサブネット内に永続的ボリュームを作成できるようにします。  a subnet to Azure NetApp Files,このステップの詳細をご覧ください。
...
サブスクリプション。:: クラスタは、 Azure NetApp Files が有効になっているサブスクリプションに含まれている必要があります。サブスクリプションはいつでも選択できます  for Azure NetApp Files,Azure NetApp Files に登録します。
VNet:: 以下の VNet の要件を考慮してください。
+
--
* クラスタは、 Azure NetApp Files 委任サブネットに直接アクセスできる VNet 内に存在する必要があります。  a subnet to Azure NetApp Files,委任されたサブネットを設定する方法について説明します。
...
に記載されているとおり  Kubernetes Service cluster requirementsでは、 Kubernetes クラスタに CSI ボリュームスナップショットコントローラがインストールされている必要があります。次の手順に従って、クラスタにコントローラをインストールします。
