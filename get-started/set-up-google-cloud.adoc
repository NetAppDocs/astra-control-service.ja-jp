...
クラスタが正常に動作していて、 1.18 ～ 1.20 の範囲の Kubernetes バージョンが実行されていること、ワーカーノードがオンラインでコンテナ最適化 OS または Ubuntu を実行していることなどを確認します。 <<GKE cluster requirements,このステップの詳細をご覧ください>>。
...
Kubernetes のバージョン:: クラスタが 1.18 ~ 1.20 の Kubernetes バージョンを実行している必要があります。
イメージタイプ::
+
--
* containderd を使用したコンテナ最適化 OS
